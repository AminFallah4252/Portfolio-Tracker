        ...
        suspend fun populateInitialData(database: AppDatabase) {
            ...
            if (profileDao.getPortfolioCount() == 0) {
                val defaultPortfolio = PortfolioProfile(
                    ...
                    description = "پورتفوی جامع طلا، سهام، تتر و ارز دیجیتال",
                    isDefault = true,
                    colorHex = "#3B82F6"
                )
                val retirementPortfolio = PortfolioProfile(
                    id = 2,
                    name = "صندوق بازنشستگی و کم‌ریسک",
                    description = "دارایی‌های امن و درآمد ثابت با افق بلندمدت",
                    isDefault = false,
                    colorHex = "#10B981"
                )
                profileDao.insertPortfolios(listOf(defaultPortfolio, retirementPortfolio))
            }
            ...
            if (categoryDao.getCategoryCount() == 0) {
                ...
                val catRealEstate = AssetCategory(
                    ...
                    maxWeight = 50.0,
                    targetTolerance = 0.0,
                    description = "دارایی‌های ملکی و مسکن (منجمد / غیرقابل نقدشوندگی سریع)",
                    sortOrder = 7
                )

                categoryDao.insertCategories(listOf(catGold, catFixed, catStock, catFx, catFiat, catCrypto, catRealEstate))
            }
        }
    }
}
